...
funnel\_analysis?: { current: number; limit: number };
};
}
...
case 'past\_due': return "bg-red-600";
...
case 'past\_due': return "Pagamento pendente";
...
return ( <Card> <CardHeader> <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3"> <div className="space-y-1"> <CardTitle className="flex items-center gap-2 flex-wrap">
Sua Assinatura <Badge className={getStatusColor(userSubscription.status)}>
{getStatusText(userSubscription.status)} </Badge>
{userSubscription.cancel\_at\_period\_end && ( <Badge variant="outline" className="border-yellow-500 text-yellow-700">
Cancelamento agendado </Badge>
)} </CardTitle> <CardDescription>
Gerencie sua assinatura e veja seu uso atual </CardDescription> </div> <div className="flex gap-2 self-end sm:self-auto"> <Button onClick={onRefresh} variant="outline" size="sm" disabled={loading} className="touch-target">
\<RefreshCw className={`h-4 w-4 mr-2 ${loading ? 'animate-spin' : ''}`} />
Atualizar </Button> <Button onClick={onManage} variant="outline" size="sm" disabled={loading} className="touch-target"> <Settings className="h-4 w-4 mr-2" />
Gerenciar </Button> </div> </div> </CardHeader> <CardContent> <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6"> <div> <h3 className="font-semibold text-gray-900">Plano Atual</h3> <p className="text-xl sm:text-2xl font-bold text-blue-600">
{userSubscription.plan?.name || "Plano não encontrado"} </p>
{userSubscription.plan?.price\_monthly !== undefined && ( <p className="text-gray-600">
{formatPrice(userSubscription.plan.price\_monthly)} / mês </p>
)} </div>
